...
def UserData(request):
    ...
    for fetch in UserFile.objects.all():
        ...
        records["student"] = fetch.username
        records["name"] = os.path.basename(fetch.files.name)
        records["size"] = fetch.files.size
        records["course"] = fetch.get_course_display()
        records["files"] = request.build_absolute_uri(fetch.files.url)
